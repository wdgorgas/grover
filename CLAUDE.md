...
- **Branch per slice:** `git checkout -b phase-pX-short-description`. Direct commits to `master` only for final verified merges.
...
- **Cowork with a mounted folder:** do not run git write-operations inside the sandbox — the mount corrupts git lock files. Prepare files with file tools; give the human exact git commands to run on the host. (Read-only git commands are fine.) In Claude Code on a normal local checkout, git works normally.
