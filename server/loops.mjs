 ...
 * enforced against a state machine, audited, and written to loop_events —
 * each loop carries its own inspectable history. This module owns its own
 * schema so extending it never touches the core db file (docs/DECISIONS.md #8).
 *
 * Two doors into a loop, one philosophy:
 *  - Greenlight: an existing ledger item → proposal → approval → loop.
 *  - Improvement Request: free text → drafted item + proposal → approval
 *    → ledger item and loop created together (or saved for later / rejected).
 */
...
export function ensureLoops() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS loop_events (
      ...
      event TEXT NOT NULL,           -- proposed | status | note
      from_status TEXT,
      to_status TEXT,
      note TEXT,
      actor TEXT,
...
/**
 * Improvement Request door: free text ("add an X to delete chats from the
 * sidebar") → drafted ledger item fields + full build proposal. Persists
 * nothing — the user approves, edits, saves, or rejects it first.
 */
...
export function setLoopStatus({ user, loopId, status, summary, reason }) {
...
export function systemStatus() {
  ...
  const queued = one(`SELECT COUNT(*) AS n FROM loops WHERE status IN ('approved','ready')`).n;
  ...
  if (loop) {
    next.push(['approved', 'ready'].includes(loop.status)
      ...
      : loop.status === 'verifying'
        ? `Verify loop #${loop.id}: ${(loop.verify_plan[0] || loop.goal).slice(0, 60)}`
  ...
  if (next.length === 0) next.push('Nothing queued — request an improvement in the Builder or log work in the Ledger.');
  return {
