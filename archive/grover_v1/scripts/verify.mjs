 ...
 * improvement-request → proposal → approval → loop lifecycle, plus direct
 * execution (Path 1), the evidence-gated done, dismissed items, and the stop
 * endpoint — without ever touching the real database. No API key is passed,
 * so proposal generation and the runner exercise the honest offline/blocked
 * paths. Zero dependencies.
 */
...
const child = spawn(process.execPath, [join(ROOT, 'grover.mjs')], {
  env: {
    ...
    GROVER_NO_OPEN: '1',
    GROVER_DATA: tmpData,
    ANTHROPIC_API_KEY: '', // force the honest offline/blocked paths
  },
  stdio: 'pipe',
});
let booted = false;
...
/** POST to an SSE endpoint, read the whole stream, return parsed events. */
...
if (!booted) {
  ...
  const checks = [
    ['/api/bootstrap', (d) => d.user && Array.isArray(d.users) && d.settings
      && d.ledgerStatuses?.includes('dismissed')],
  ...
  console.log('\nImprovement Request → loop lifecycle (Path 2 machinery):');
  try {
    ...
    // Full happy path: approved → running → verifying → done (evidence-gated)
    ...
    // The DONE gate: no evidence, no done.
    r = await post(`/api/loops/${loop.id}/status`, { status: 'done', summary: 'shortcut shipped; verified by hand' });
    check('done WITHOUT evidence rejected', r.status === 400);
    r = await post(`/api/loops/${loop.id}/status`, {
      status: 'done', summary: 'shortcut shipped; verified by hand',
      manual_evidence: 'added the keydown handler by hand, ran verify, exercised the shortcut in the browser',
    });
    check('done WITH manual evidence accepted', r.status === 200 && r.data.loop.status === 'done'
      && r.data.loop.execution_evidence?.manual === true
      && typeof r.data.loop.execution_evidence.text === 'string');
  ...
  // ---- Mutation battery: Direct execution (Path 1) --------------------------------
  console.log('\nDirect execution (Path 1):');
  try {
    // Bad request first
    let r = await post('/api/execute', {});
    check('execute without request rejected', r.status === 400);

    // No API key → ledger item + loop created, run blocked honestly, no fake evidence
    const { status: xStatus, events } = await sse('/api/execute', { request: 'add a tooltip to the send button' });
    const loopEv = events.find((e) => e.type === 'loop');
    const doneEv = events.find((e) => e.type === 'done');
    check('execute streams loop + done events', xStatus === 200 && Boolean(loopEv) && Boolean(doneEv));
    check('no key → run blocked honestly', doneEv?.status === 'blocked' && /api key/i.test(doneEv?.reason || ''));
    const dLoop = (await get('/api/loops')).data.find((l) => l.id === loopEv?.loopId);
    check('direct loop persisted (source direct, blocked with reason)', dLoop
      && dLoop.source === 'direct' && dLoop.status === 'blocked'
      && /api key/i.test(dLoop.blocked_reason || ''));
    check('runner attached no fake evidence', !dLoop?.execution_evidence);
    const dItem = (await get('/api/ledger')).data.find((i) => i.id === loopEv?.ledgerId);
    check('direct item approved, never pending_greenlight', dItem?.status === 'approved'
      && dItem.domain === 'grover-dev');
    r = await get(`/api/loops/${loopEv?.loopId}/events`);
    check('direct loop has a birth event', r.status === 200 && r.data.length >= 1);

    // Stop endpoint answers (nothing running → honest wasRunning:false)
    r = await post(`/api/loops/${loopEv?.loopId}/stop`, {});
    check('stop endpoint answers', r.status === 200 && r.data.ok === true && r.data.wasRunning === false);
  } catch (e) { bad('direct execution', e.message); }
  ...
  // ---- Mutation battery: dismissed status ----------------------------------------
  console.log('\nLedger dismissed status:');
  try {
    const pend = (await get('/api/ledger')).data.find((i) => i.status === 'pending_greenlight');
    let r = await post(`/api/ledger/${pend.id}/status`, { status: 'dismissed' });
    check('dismiss a pending item', r.status === 200);
    const dismissed = (await get('/api/ledger')).data.find((i) => i.id === pend.id);
    check('dismissed persisted', dismissed?.status === 'dismissed');
    r = await post(`/api/ledger/${pend.id}/status`, { status: 'not_a_status' });
    check('bad ledger status still rejected', r.status !== 200);
  } catch (e) { bad('dismissed status', e.message); }
}
