 ...
 *
 * Drag-and-drop: ledger/loop cards can be dropped onto the chat log — the
 * item becomes the subject of the next message (a one-line reference is
 * prefixed so Grover has the context).
 */
...
export class Chat {
  constructor({ orb, state }) {
    ...
    this.mode = 'chat';
    this.busy = false;
    this.subject = null;      // dropped ledger item riding along on the next message
  ...
  bind() {
    ...
    // Drop a ledger/loop card here to make its item the subject of conversation.
    this.log.addEventListener('dragover', (e) => {
      if ([...e.dataTransfer.types].includes('application/x-grover-item')) {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'copy';
      }
    });
    this.log.addEventListener('drop', (e) => this.handleItemDrop(e));
  }

  async handleItemDrop(e) {
    const id = Number(e.dataTransfer.getData('application/x-grover-item'));
    if (!id) return;
    e.preventDefault();
    try {
      const items = await api('/api/ledger');
      const it = items.find((x) => x.id === id);
      if (!it) { toast(`Ledger item #${id} not found.`, 'err'); return; }
      this.subject = it;
      this.subjectCard?.remove();
      this.subjectCard = this.addCard(`
        <h4>Subject: ledger #${it.id}</h4>
        <div class="small dim">${esc(it.item)} — ${esc(it.status.replace(/_/g, ' '))} · ${esc(it.domain)} · ${esc(it.severity)}</div>
        ${it.notes ? `<div class="faint small" style="margin-top:4px">${esc(it.notes.slice(0, 200))}</div>` : ''}
        <div class="faint small" style="margin-top:6px">Attached to your next message.
          <button class="ghost" data-x>✕ Detach</button></div>`);
      this.subjectCard.querySelector('[data-x]').onclick = () => this.clearSubject();
      this.input.focus();
  ...
  renderGreeting() {
    ...
    this.addCard(`<h4>GROVER online.</h4>
      ...
      Every turn is routed, costed, and budget-gated. Drop a ledger card here to talk about it.</div>`);
  }
  ...
  async send(forceBody = null) {
    if (this.busy) return;
    let message = forceBody ? forceBody.message : this.input.value.trim();
    if (!message) return;
    if (!this.state.hasApiKey) { this.renderKeySetup(); return; }

    // A dropped item rides along on the next message as a one-line reference.
    if (!forceBody && this.subject) {
      message = `[Re: ledger item #${this.subject.id} "${this.subject.item}" — ${this.subject.status.replace(/_/g, ' ')}, ${this.subject.domain}]\n${message}`;
      this.clearSubject();
    }
