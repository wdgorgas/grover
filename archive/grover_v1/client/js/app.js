...
/**
 * Waiting-to-run loops as small draggable widgets right in the Command
 * Center — drag one onto the chat log to make it the subject of
 * conversation (chat.js already handles the drop). Falls back to the plain
 * next-actions text when there's nothing queued yet.
 */
function renderQueueWidgets(queueEl, s) {
  ...
  queueEl.innerHTML = items.map((l) => `
    <div class="queue-card" draggable="true" data-drag-item="${l.ledger_id}"
      title="Drag onto the chat below to workshop it">
      <span class="queue-card-goal">${escText(l.goal.slice(0, 64))}</span>
      <span class="queue-card-meta">#${l.id} · ${escText(l.status)}</span>
    </div>`).join('');
  queueEl.querySelectorAll('[data-drag-item]').forEach((card) => {
