...
> Read JACKSON_START_HERE.md, then README.md, then grover_v2_handoff.md, then grover_v2_scope_understanding.md, then every file in chatgpt_handoffs/ in numeric order, then PLANNING_BOARD.md. Then summarize the current state and my active assignment back to me before doing anything.
...
2. Check `PLANNING_BOARD.md` for your assignment. Your suggested first one: **workstream 2, the UI/UX spec track** (visual direction, theme tokens, Command Center/Builder layout). Claim it by putting your name in the board and pushing.
...
5. Save outputs to the location listed on the board (e.g., `chatgpt_handoffs/jackson_01_uiux_spec.md`).
...
- **Don't touch the `iter_NN` files** in `chatgpt_handoffs/` — that's the main thread, owned per the board. Your files are `jackson_NN_<topic>.md`.
...
- **Locked decisions** (bottom of `PLANNING_BOARD.md`) are settled — don't relitigate them; spend your energy on open questions.
...
- Taste anchor for the UI/UX track: Linear / Arc / Things / Raycast — artistry, modern, smooth, human. Explicitly rejected: Iron-Man HUD, glowing circuits, generic dark-SaaS AI chrome. The particle orb is untouchable — it carries over exactly as-is. References live in `ART INSPIRATION/` and `GROVER command center UI design/`.
